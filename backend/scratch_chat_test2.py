...
def chat(token):
    ...
    req = urllib.request.Request(f"{BASE_URL}/chat", data=json.dumps({"message": "اخبرني عن العميل Tonya Johnson", "session_id": session_id, "language": "ar"}).encode('utf-8'), headers={'Content-Type': 'application/json', 'Authorization': f'Bearer {token}'}, method="POST")
    try:
        with urllib.request.urlopen(req) as response:
